...
class Picture:
	...
	def getPixel( x, y):
		loc = (IMGWIDTH * y) + x
		return [(self.image[loc]),(self.image[loc+1]),(self.image[loc+2])]		
...
class Writer:
	...
	def takePicture(self):
		pic = Picture()	
		pic.image  = create_string_buffer((pic.IMGWIDTH * pic.IMGHIEGHT * 3)+20)
		self.serialLib.fluke_get_image(pic.image)
		return pic
	...
	def getObstacle2(self,value = None):
	...
	def getObstacle(self,*pos):
		return int(self.serialLib.fluke_get_ir_center()) 
...
def get(sensor = "all", *position):
	...
	if(sensor == "config"):
		...
		#returns a 3 tuple 
		writer.write([GET_BLOB])
		red = writer.read(PACKET_LENGTH+4)
		red = red[9:]
		hbyte = red[0]
		lbyte = red[1]
		lbyte = (hbyte << 8 | lbyte)

		return (lbyte,red[2],red[3])
...
def getAll():
	...
	retval =  writer.read(PACKET_LENGTH + 11)[9:]

	return {"light": [retval[2] << 8 | retval[3], retval[4] << 8 | retval[5], retval[6] << 8 | retval[7]], "ir": [retval[0], retval[1]], "line": [retval[8], retval[9]], "stall": retval[10],"obstacle": [getObstacle("left"), getObstacle("center"), getObstacle("right")],"blob": getBlob(),"battery": getBattery(),}
...
def getLight(*pos):
	writer.write([GET_LIGHT_ALL])
	red = writer.read(PACKET_LENGTH + 6)
	red = red[9:]
	retval = []
	for i in range(0,5,2):
		retval.append(red[i] << 8 | red[i+1])			
	return retval
